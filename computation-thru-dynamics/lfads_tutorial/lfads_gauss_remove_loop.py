...
def lfads_params(key, lfads_hps):
  ...
  con_params = gru_params(next(skeys), con_dim, 2*enc_dim + ii_dim)
...
def lfads_decode_one_step(params, lfads_hps, key, keep_rate, c, ii, g, g_approx, xenc):
  ...
  cin = np.concatenate([xenc, ii], axis=0)
